...
document.addEventListener('DOMContentLoaded', () => {
  ...
  async function updateSunriseTime() {
    try {
      ...
      // Fetch sunrise with formatted=1 (local time)
      const sunResponse = await fetch(`https://api.sunrise-sunset.org/json?lat=${lat}&lng=${lon}&formatted=1`);
      const sunData = await sunResponse.json();

      if (sunData.status === "OK") {
        // API returns time like "5:30:00 AM"
        const sunriseLocal = sunData.results.sunrise;
        const sunriseLocalTrimmed = sunriseLocal.replace(/:\d{2} /, ' ');


        document.getElementById('sunriseTime').innerHTML = `Sunrise:<br>${sunriseLocalTrimmed}`;
      } else {
...
// Convert "1:45 PM" to a Date object
...
// Determine next upcoming prayer based on current time and day
...
// Update highlights and countdown timer
...
setInterval(updateHighlightAndCountdown, 1000);
 ...
 async function loadHadithsAndScroll() {
  ...
  function scrollLine() {
    ...
    scrollTextLabel.style.position = 'absolute'; // ensures exact positioning
    ...
    requestAnimationFrame(() => {
      ...
      const speed = 1.2; // pixels per frame — adjust for faster/slower scroll
      ...
      function step() {
        ...
        if (leftPos + textWidth > 0) {
          ...
          // Add a short delay before next scroll
